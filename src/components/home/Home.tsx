...
const Home:React.FC = () => {
	...
	return (
		<div className={`flex-col sm:h-[1200px] justify-around items-center ${state.darkMode ? "bg-gradient-to-t from-stone-950 to-amber-900 text-white" : "bg-gradient-to-t from-amber-500 to-amber-200 text-slate-900"}`}>
			<div className="flex flex-col sm:flex-row sm:items-center sm:h-[50%] sm:justify-around 2xl:justify-around 2xl:mx-56 pt-6 sm:mx-10">
				...
				<div className="flex flex-col items-center py-4 ml-4 gap-4 2xl:text-xl">
					<motion.h1 initial={{x:`${document.body.clientWidth < 500 ? 500 : 1200}`}} animate={{x:0}} transition={{delay: 0.5, duration: 1}} className="text-center text-2xl 2xl:text-3xl mb-2 font-bold tracking-wider">{state.language == "ENG" ? "Welcome to Meditation Wave" : "Benvenuto su Meditation Wave"}</motion.h1>
					...
					<motion.button initial={{x:`${document.body.clientWidth < 500 ? 500 : 1200}`}} animate={{x:0}} transition={{delay: 0.5, duration: 1.8}} onClick={() => navigate("controller")} className="w-24 sm:w-28 h-8 sm:h-10 text-sm sm:text-lg bg-amber-800 hover:bg-amber-900 rounded-2xl text-white font-medium tracking-widest mt-4">START</motion.button>
				</div>
			</div>
			<div className="flex flex-col sm:flex-row-reverse sm:items-center sm:h-[50%] sm:justify-around 2xl:justify-around 2xl:mx-56 pt-6 sm:mx-10">
				...
				<div className="flex flex-col items-center py-4 gap-4 2xl:text-xl">
